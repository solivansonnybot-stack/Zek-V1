...
try:
    ...
    print("[ADA] PyAudio not available - running in server mode (audio via WebSocket only)")
import PIL.Image
import mss
import argparse
import math
...
config = types.LiveConnectConfig(
    ...
    system_instruction="Your name is Jake, an advanced AI assistant. "
        "You have a calm, confident and authoritative personality. "
        "Speak in a deep, composed and intelligent tone. "
        "When answering, respond using complete and concise sentences to keep a quick pacing and keep the conversation flowing. "
        "You are highly capable and always ready to assist.",
    tools=tools,
...
pya = pyaudio.PyAudio() if PYAUDIO_AVAILABLE else None
...
class AudioLoop:
    ...
    async def listen_audio(self):
        ...
        if not PYAUDIO_AVAILABLE:
            print('[ADA] PyAudio not available - skipping local mic input (server mode)')
            return
        try:
                            ...
                            if transcript:
                                ...
                                if transcript != self._last_output_transcription:
                                    ...
                                    if delta:
                                        # Send to frontend (Streaming)
                                        if self.on_transcription:
                                             self.on_transcription({"sender": "JAKE", "text": delta})
                                        
                                        # Buffer for Logging
                                        if self.chat_buffer["sender"] != "JAKE":
                                            # Flush previous
                                            if self.chat_buffer["sender"] and self.chat_buffer["text"].strip():
                                                self.project_manager.log_chat(self.chat_buffer["sender"], self.chat_buffer["text"])
                                            # Start new
                                            self.chat_buffer = {"sender": "JAKE", "text": delta}
                                        else:
                                            # Append
                                            self.chat_buffer["text"] += delta
    ...
    async def play_audio(self):
        if not PYAUDIO_AVAILABLE:
            # Server mode: only forward audio via callback (WebSocket), no local playback
            while True:
