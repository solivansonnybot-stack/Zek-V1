...
# CORS para permitir acesso do frontend web hospedado em qualquer origem
...
# Servir frontend web estático se existir
...
@app.get("/")
async def serve_web_frontend():
    html_path = pathlib.Path(__file__).parent.parent / "web_frontend.html"
    if html_path.exists():
        return FileResponse(str(html_path))
    return {"status": "Jake Backend Running", "version": "2.0"}
...
async def audio_input(sid, data):
    """Recebe chunks de áudio do browser (Int16 array) e envia para o Gemini."""
    global audio_loop
    if audio_loop and audio_loop.session:
        try:
            ...
            if samples:
                ...
                asyncio.create_task(
                    audio_loop.session.send_realtime_input(
                        audio=types.Blob(data=raw_bytes, mime_type='audio/pcm;rate=16000')
                    )
                )
        except Exception as e:
            print(f'[SERVER] Error forwarding audio input: {e}')
